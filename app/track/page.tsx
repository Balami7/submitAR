...
function TrackContent() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    setError('');
    setOrder(null);
    if (!csn.trim() || !contact.trim()) {
      setError('Please enter both your order number and phone/email.');
      return;
    }
    setLoading(true);
    try {
      const res = await fetch(
        `/api/track?csn=${encodeURIComponent(csn.trim())}&contact=${encodeURIComponent(contact.trim())}`
      );
      ...
      <div className="min-h-screen bg-gray-50 pt-10 pb-16 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-gray-600 mb-8">
            Enter your order number (CSN) and the phone number or email you used.
          </p>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Phone or email on the order"
                  className="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500"
                />
              </div>
            </div>
